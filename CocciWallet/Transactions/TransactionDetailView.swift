...
struct TransactionDetailView: View {
    ...
    var with: String {
        isOutGoing ? tx.to.string : tx.from.string
    }
    
    var isOutGoing: Bool {
        address == tx.from.string.lowercased()
    }
...
extension TransactionDetailView.Cell {
    init(tx: any TransactionProtocol, price: Double?, address: String) {
        ...
        let title = address == tx.to.string ? "Transfer From" : "Transfer To"
...
#Preview {
    TransactionDetailView(tx: Transaction.generatedDummyData.first!, address: Wallet.rifigy.address.string)
}
