...
@Observable
class SendViewModel {
    let evm: Web3Kit.EthereumNetwork
    var address: String
    let decimals: UInt8
    ...
    init(evm: Web3Kit.EthereumNetwork, address: String, decimals: UInt8) {
        self.evm = evm
        self.address = address
        self.decimals = decimals
    }
    
    func fetchGas(client: EthClient) async {
    ...
    func fetchGasEstimate(value: BigUInt?, client: EthClient) async {
